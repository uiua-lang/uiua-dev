...
impl Compiler {
    ...
    fn modifier_ref(
        ...
        let node = if let Some(mut mac) = self.index_macros.get(&local.index).cloned() {
            ...
            let op_sigs = if mac.function.sig().args == 2 {
            ...
            (|| -> UiuaResult {
                if let Some(index) = node_unbound_index(&self.asm[&mac.function], &self.asm) {
...
fn node_unbound_index(node: &Node, asm: &Assembly) -> Option<usize> {
    match node {
        ...
        Node::Call(func) => node_unbound_index(&asm[func], asm),
        _ => None,
    }
}
